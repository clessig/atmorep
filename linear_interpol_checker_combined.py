...
if __name__ == '__main__':
    ...
    # run_ids =["10718660","10718661","10718662","10718663","10718664"]
        ...
        for idx_level, level in enumerate(levels): # to be re-adjusted
            ...
            for isample in range(len(da_target_dc)):
                ...
                for itoken in da_target_dc[isample].itoken.values:
                    ...
                    ds_o['interpol'].loc[ dict( datetime=datetime_target_t, 
                                    lat=lat_target_t, lon=lon_target_t) ] = copy.deepcopy(interpol.loc[dict(datetime=datetime_target_t, lat=lat_target_t, lon=lon_target_t)])
                # selecting lon-lat and filling time-wise
                for itoken_surf in [4, 6, 9, 11]:
                    lat_target_t = da_target_dc[isample][itoken_surf].lat.values
                    lon_target_t = da_target_dc[isample][itoken_surf].lon.values
                    to_be_interpol = interpol.sel(lat = lat_target_t,lon = lon_target_t)
                    got_interpol = to_be_interpol.interpolate_na(dim="datetime", method="linear")
                    got_extrapol = got_interpol.ffill(dim="datetime") 
                    interpol.loc[dict(lat=lat_target_t, lon=lon_target_t, datetime=got_extrapol.datetime)] = got_extrapol
                    ds_o['interpol'].loc[ dict(lat = lat_target_t, lon = lon_target_t, datetime=got_extrapol.datetime)] = got_extrapol

            lon_=  np.linspace( 0., 360., num=360*4, endpoint=False)
            for datetime_ in datetime_all:
                to_be_interpol2 = ds_o['interpol'].sel(
                    datetime=datetime_,
                    lon=lon_
                )
                ...
                ds_o['interpol'].loc[ dict( datetime=datetime_, lon=lon_, lat=got_interpol2.lat)] = got_extrapol2
