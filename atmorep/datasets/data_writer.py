...
#
#  Copyright (C) 2022
#{"IsDistinguishedFolder":true,"FolderId":{"Id":"AQMkAGJmNjU3NGJjLTcwNzAtNGU3Yi04Y2YyLWExNTc0OWJkM2M1YwAuAAAD/KFws63DUUiD+lOilV38egEA6TSkqfNZpUyM1x48YZ8/EQAAAgEPAAAA","ChangeKey":"AQAAABYAAADpNKSp81mlTIzXHjxhnz8RAABodWru"},"DragItemType":3}
####################################################################################################
#
#  project     : atmorep
#
...
def write_BERT( model_id, epoch, batch_idx, levels, sources, 
  ...
  '''temporarly masked until I get a better data writing function'''
  store_ens = zarr_store( fname.format( 'ens'))
  exp_ens = zarr.group(store=store_ens)
  for fidx, field in enumerate(ensembles) :
    if 0 == len(field[1]) :  # skip fields that were not predicted
...
####################################################################################################
def write_data_compression_global( model_id, epoch, batch_idx, levels, masked_levels, sources, 
                    targets, preds, ensembles, coords,  
                    zarr_store_type = 'ZipStore' ) :
  ''' 
    sources : num_fields x [field name , data]
...
####################################################################################################
def write_data_compression_BERT( model_id, epoch, batch_idx, levels, sources, 
                targets, preds, ensembles, coords, 
                zarr_store_type = 'ZipStore' ) : 
