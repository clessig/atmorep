...
class PerceiverCrossAttentionHead(torch.nn.Module):
  ...
  def __init__(self, dim_embed, coupled_emb_dims, perceiver_latent_dimension, dropout_rate = 0., with_qk_lnorm =False,
    ...
    self.proj_latent = torch.nn.Linear( dim_embed, perceiver_latent_dimension, bias=False)

    self.proj_kv= torch.nn.Linear(dim_embed,perceiver_latent_dimension*2,bias=False)

    self.proj_kv_coupled = torch.nn.ModuleList()

    if coupled_emb_dims is not None:
        for coupled_emb_dim in coupled_emb_dims:
            self.proj_kv_coupled.append( torch.nn.Linear( coupled_emb_dim, perceiver_latent_dimension*2, bias=False))
    ...
    self.proj_out = torch.nn.Linear( perceiver_latent_dimension, perceiver_latent_dimension, bias=False)
...
class Perceiver(torch.nn.Module) :
    ...
    def create( self):
        ...
        self.cross_attn = PerceiverCrossAttentionHead(dim_embed, coupled_emb_dims, cf.perceiver_latent_dimension, cf.dropout_rate, with_qk_lnorm = cf.with_qk_lnorm, grad_checkpointing = cf.grad_checkpointing)
        ...
        self.output_latents = torch.nn.Parameter(torch.empty(np.prod(output_shape), cf.perceiver_latent_dimension)) # cf.perceiver_output_emb))

        if cf.init_scale > 0.0:
            with torch.no_grad():
                self.output_latents.normal_(0.0,cf.init_scale)

        self.output_cross_attn = PerceiverCrossAttentionHead(cf.perceiver_latent_dimension, [], cf.perceiver_latent_dimension, cf.dropout_rate, with_qk_lnorm=cf.with_qk_lnorm, grad_checkpointing=cf.grad_checkpointing)
